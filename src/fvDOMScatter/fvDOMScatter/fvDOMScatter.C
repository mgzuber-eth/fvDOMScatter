...
{
    defineTypeNameAndDebug(fvDOMScatter, 0);
    addToRadiationRunTimeSelectionTables(fvDOMScatter);
}
}
...
void Foam::radiationModels::fvDOMScatter::initialise()
{
    ...
    if (mesh_.nSolutionD() == 3)
    {
        ...
        for (label n = 1; n <= nTheta_; n++)
        {
            for (label m = 1; m <= 4*nPhi_; m++)
            {
                ...
                (
                    i,
                    new radiativeIntensityRayScatter
                    (
                        *this,
                        mesh_,
                        phii,
...
Foam::radiationModels::fvDOMScatter::fvDOMScatter(const volScalarField& T)
:
...
Foam::radiationModels::fvDOMScatter::fvDOMScatter
(
...
bool Foam::radiationModels::fvDOMScatter::read()
{
...
void Foam::radiationModels::fvDOMScatter::calculate()
{
    ...
    do
    {
        ...
        forAll(IRay_, rayI)
        {
            if (!rayIdConv[rayI])
            {
                scalar maxBandResidual = IRay_[rayI].correct(nRay_, IRay_, rayI); //fvDOMScatter
...
Foam::tmp<Foam::volScalarField> Foam::radiationModels::fvDOMScatter::Rp() const
{
...
Foam::radiationModels::fvDOMScatter::Ru() const
{
...
void Foam::radiationModels::fvDOMScatter::updateBlackBodyEmission()
{
    for (label j=0; j < nLambda_; j++)
    {
...
void Foam::radiationModels::fvDOMScatter::updateG()
{
...
void Foam::radiationModels::fvDOMScatter::setRayIdLambdaId
(
    const word& name,
    label& rayId,
    label& lambdaId
