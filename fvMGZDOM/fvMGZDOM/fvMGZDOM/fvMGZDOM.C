...
namespace Foam
{
    namespace radiation
    {
        defineTypeNameAndDebug(fvMGZDOM, 0);
        addToRadiationRunTimeSelectionTables(fvMGZDOM);
    }
}
...
void Foam::radiation::fvMGZDOM::initialise()
{
    // 3D
    ...
    Info<< "fvMGZDOM : Allocated " << IRay_.size()
    ...
    if (useSolarLoad_)
    {
        ...
        if (solarLoad_->nBands() > 1)
        {
            FatalErrorInFunction
                << "Requested solar radiation with fvMGZDOM. Using "
...
Foam::radiation::fvMGZDOM::fvMGZDOM(const volScalarField& T)
:
...
Foam::radiation::fvMGZDOM::fvMGZDOM
(
...
bool Foam::radiation::fvMGZDOM::read()
{
...
void Foam::radiation::fvMGZDOM::calculate()
{	
    ...
    do
    {
        ...
        forAll(IRay_, rayI)
        {
            if (!rayIdConv[rayI])
            {				
				scalar maxBandResidual = IRay_[rayI].correct(nRay_, IRay_, rayI); //MGZ
...
Foam::tmp<Foam::volScalarField> Foam::radiation::fvMGZDOM::Rp() const
{
...
Foam::radiation::fvMGZDOM::Ru() const
{
...
void Foam::radiation::fvMGZDOM::updateBlackBodyEmission()
{
    for (label j=0; j < nLambda_; j++)
    {
...
void Foam::radiation::fvMGZDOM::updateG()
{
...
void Foam::radiation::fvMGZDOM::setRayIdLambdaId
(
    const word& name,
    label& rayId,
    label& lambdaId
