...
#include "radiativeMGZIntensityRay.H"
#include "fvm.H"
#include "fvMGZDOM.H"
#include "constants.H"
...
const Foam::word
Foam::radiation::radiativeMGZIntensityRay::intensityPrefix("ILambda");


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::radiation::radiativeMGZIntensityRay::radiativeMGZIntensityRay
(
    const fvMGZDOM& dom,
    const fvMesh& mesh,
    const scalar phi,
    ...
    const blackMGZBodyEmission& blackBody,
    const label rayId
)
:
    dom_(dom),
...
Foam::scalar Foam::radiation::radiativeMGZIntensityRay::correct
(
	const scalar nRay_, //MGZ
	PtrList<radiativeMGZIntensityRay>& allRays, //MGZ
	const scalar currentRay //MGZ
)
{
	...
	//MGZ Step 1: attentuation by scattering
	//make dummy variable (for now) for scattering coefficient
	const dimensionedScalar sigmaDummy
	(
		"sigmaDummy",
    ...
    forAll(ILambda_, lambdaI)
    {
		...
		//MGZ Step 2: augmentation by scattering
		...
		for (label rayI=1; rayI < nRay_; rayI++)
		{
			...
			//Info << "this here: " << allRays[rayI].ILambda(lambdaI)() << nl;
		}
		...
		volScalarField siakam ("siakam", 0.*blackBody_.bLambda(0));
		//scalar cosAngle;
		scalar muS;
		scalar phaseFunc;
		//scalar radians;
		for (label rayI=0; rayI < nRay_; rayI++)
		{
			...
			siakam.ref() += allRays[rayI].ILambda(lambdaI)()*allRays[rayI].omega()*phaseFunc;
			//Info << "phaseFunc: " << phaseFunc << nl;
			//Info << "omega: " << allRays[rayI].omega() << nl;
			//radians = acos(muS);
		}
		
		//Info << "siakam: " << siakam << nl;
		...
		(
		  ...
		  + fvm::Sp(sigmaDummy*omega_, ILambda_[lambdaI]) //MGZ
		==
		  ...
		  + sigmaDummy*omega_/(4.0*constant::mathematical::pi)*siakam //MGZ
		);
...
void Foam::radiation::radiativeMGZIntensityRay::addIntensity()
{
